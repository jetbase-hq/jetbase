...
def test_generate_new_migration_file_cmd_success(tmp_path, capsys):
    ...
    with patch("jetbase.commands.new.find_jetbase_directory", return_value=tmp_path):
...
def test_generate_new_migration_file_cmd_directory_not_found():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch(
            "jetbase.commands.new.find_jetbase_directory", return_value=tmp_path
        ):
...
def test_generate_new_migration_file_cmd_jetbase_not_found():
    """Test that DirectoryNotFoundError is raised when jetbase directory doesn't exist."""
    with patch("jetbase.commands.new.find_jetbase_directory", return_value=None):
        with pytest.raises(DirectoryNotFoundError) as exc_info:
            generate_new_migration_file_cmd("create users table")
